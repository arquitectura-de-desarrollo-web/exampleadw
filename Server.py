...
def server_program():
    ...
    while True:
        ...
        t1 = threading.Thread(target=suma, args=(int(aux[0]),my_queue)) 
        t2 = threading.Thread(target=suma, args=(int(aux[1]),my_queue))
        t3 = threading.Thread(target=suma, args=(int(aux[2]),my_queue)) 
        t4 = threading.Thread(target=suma, args=(int(aux[3]),my_queue))  
        ...
        conn.send(str(total2).encode())  # envía el total de la suma al cliente
